"""Location database for the Cost of Living Comparison Tool.

Maps CBSA codes to metro area names, FIPS codes to county names, and
state FIPS codes to state names. These are the locations supported by
MIT's Living Wage Calculator (https://livingwage.mit.edu).

To add a location, find its code at the MIT site and add an entry to
the appropriate dictionary below.
"""
